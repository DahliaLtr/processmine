...
def parse_log_with_mapping(log_text: str, mapping: list[dict]) -> pd.DataFrame:
    ...
    sorted_mapping = sorted(mapping, key=lambda x: len(x.get('keyword','0')), reverse=True)
    
    begin_kw  = next((m['keyword'] for m in mapping if m['activity'] == 'Start Batch'), 'BEGIN')
    end_kw    = next((m['keyword'] for m in mapping if m['activity'] == 'End Process'), 'END')
    case_kw   = next((m.get('case_keyword','') for m in mapping if m.get('case_keyword')), '')
...
def compute_bottleneck_table(df_dur: pd.DataFrame) -> pd.DataFrame:
    """Compute per-activity performance metrics."""
    exclude = ['Start Batch', 'End Process', 'End Batch', 'Start Process']
    df_f = df_dur[~df_dur['concept:name'].isin(exclude)]
    
    grp = df_f.groupby('concept:name')['duration_ms'].agg(
        frekuensi='count',
        total_waktu='sum',
        rata_rata='mean',
        median='median',
...
def compute_transition_delay(df_dur: pd.DataFrame) -> pd.DataFrame:
    ...
    result = df_s.groupby('transition')['duration_ms'].agg(
        frekuensi='count', rata_rata='mean', median='median', p95=lambda x: x.quantile(0.95)
    ).reset_index()
...
def make_plotly_bar(df, x_col, y_col, title, color='#00d4ff', h=400):
    ...
    fig = go.Figure(go.Bar(
        x=df_plot[x_col],
        y=df_plot[y_col].str[:35] + ('…' if df_plot[y_col].str.len().max() > 35 else ''),
        orientation='h',
        marker=dict(
            color=df_plot[x_col],
...
plot_bgcolor='#F5F1EB',
        ...
        height=h,
        yaxis=dict(autorange='reversed', gridcolor='#D6C2A8', tickfont=dict(size=11, family='Space Mono')),
...
def fitness_gauge(value: float, label: str):
    color = '#00ff9d' if value >= 0.8 else '#ffb800' if value >= 0.5 else '#ff4757'
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=round(value * 100, 1),
        number=dict(suffix="%", font=dict(size=32, family='Space Mono', color='#e2eaf4')),
        title=dict(text=label, font=dict(size=13, family='DM Sans', color='#8fa3bc')),
        gauge=dict(
            axis=dict(range=[0,100], tickcolor='#1e2a38', tickfont=dict(color='#4a6278')),
            bar=dict(color=color),
            bgcolor='#0f1218',
            bordercolor='#1e2a38',
            steps=[
                dict(range=[0,50],  color='#0a0c10'),
                dict(range=[50,80], color='#0f1218'),
                dict(range=[80,100],color='#111820'),
            ],
            threshold=dict(line=dict(color=color, width=3), thickness=0.8, value=value*100)
        )
    ))
    fig.update_layout(
        paper_bgcolor='#151b24', font=dict(family='DM Sans', color='#8fa3bc'),
...
def run_discovery_viz(df: pd.DataFrame):
    ...
    gviz = hn_viz.apply(heu_net, parameters={
        hn_viz.Variants.PYDOTPLUS.value.Parameters.FORMAT: 'png'
    })
...
def generate_bpi_recommendations(bottleneck_df: pd.DataFrame, 
                                   deviations: list, 
                                   fitness: float) -> pd.DataFrame:
    ...
    if fitness is not None:
        fit_val = fitness.get('average_trace_fitness', 0) if isinstance(fitness, dict) else 0
        if fit_val < 0.8:
            recs.append({
                'Prioritas': '🔴 Tinggi',
...
def export_summary_csv(stats: dict, bottleneck_df: pd.DataFrame,
                       transition_df: pd.DataFrame, bpi_df: pd.DataFrame) -> bytes:
    output = io.StringIO()
...
defaults = {
    'df_raw': None, 'df_clean': None, 'df_dur': None,
    'mapping': [], 'log_stats': {},
    'bottleneck_df': None, 'transition_df': None,
    'fitness': None, 'precision': None, 'deviations': [],
    'discovery_img': None, 'bpi_df': None,
    'step': 0,  # 0=upload, 1=mapping, 2=parsed, 3=analyzed
    'dep_thresh': 0.8, 'and_thresh': 0.65,
}
...
if uploaded_log:
    ...
    if uploaded_log.name.endswith('.csv'):
        try:
            ...
            case_col  = next((c for c in df_csv.columns if any(k in c.lower() for k in ['case','id','batch'])), df_csv.columns[0])
            act_col   = next((c for c in df_csv.columns if any(k in c.lower() for k in ['activity','event','action','name'])), df_csv.columns[1] if len(df_csv.columns)>1 else df_csv.columns[0])
            ts_col    = next((c for c in df_csv.columns if any(k in c.lower() for k in ['time','timestamp','date','ts'])), df_csv.columns[2] if len(df_csv.columns)>2 else df_csv.columns[0])
            
            df_csv = df_csv.rename(columns={case_col:'case:concept:name', act_col:'concept:name', ts_col:'time:timestamp'})
            df_csv['time:timestamp'] = pd.to_datetime(df_csv['time:timestamp'], errors='coerce')
            df_csv = df_csv.dropna(subset=['case:concept:name','concept:name','time:timestamp'])
            df_csv = df_csv.sort_values(['case:concept:name','time:timestamp']).reset_index(drop=True)
            st.session_state.df_raw = df_csv
            st.session_state.step = 2
            st.success(f"✓ CSV parsed: {len(df_csv)} events, {df_csv['case:concept:name'].nunique()} cases")
        ...
        st.success(f"✓ File loaded: **{uploaded_log.name}** ({len(log_text):,} characters, {len(log_text.splitlines()):,} lines)")
...
if st.session_state.step >= 1 and not (uploaded_log and uploaded_log.name.endswith('.csv')):
    st.markdown("""<div class="section-header">
    ...
    with tab_template:
        ...
        template_repayment = [
            ...
            {"keyword":"File name timestamp","activity":"Get File Name",           "dedup": False},
            {"keyword":"batchMode",          "activity":"Check Batch Mode",        "dedup": False},
            {"keyword":"No repayment files found","activity":"No File Found",      "dedup": False},
            {"keyword":"File get",           "activity":"Download File",           "dedup": False},
            {"keyword":"Processing repayment","activity":"Process Repayment File", "dedup": False},
            {"keyword":"Populate applicationNumbers","activity":"Extract App Number","dedup": False},
            {"keyword":"There are",          "activity":"Count Data",              "dedup": False},
            {"keyword":"CreateLoan",         "activity":"Get Loan Data",           "dedup": False},
            {"keyword":"applicationNumbersIDV","activity":"Split IDV & BU",        "dedup": False},
            {"keyword":"RealizationCoBorrowing","activity":"Get Realization",       "dedup": False},
            {"keyword":"PartnerFeedBackFile","activity":"Get Partner Feedback",    "dedup": False},
            {"keyword":"NotifikasiData",     "activity":"Get Notification",        "dedup": False},
            {"keyword":"existingRepaymentFiles","activity":"Check Existing Data",  "dedup": False},
            {"keyword":"SELECT batchNumber", "activity":"Generate Batch Number",   "dedup": False},
            {"keyword":"Populate repayments","activity":"Populate Repayments",     "dedup": False},
            {"keyword":"Processing",         "activity":"Process Data",            "dedup": False},
            {"keyword":"DELETE FROM RepaymentFile","activity":"Delete Old Data",   "dedup": False},
            {"keyword":"INSERT INTO RepaymentFile","activity":"Insert Data",       "dedup": False},
            {"keyword":"Total files processed","activity":"Summary",               "dedup": False},
            {"keyword":"saveMonitoringDetail","activity":"Save Monitoring",        "dedup": False},
            {"keyword":"NOTIF NUMBER",       "activity":"Process Notification",    "dedup": True},
        ...
        with col_t3:
            tpl_df = pd.DataFrame([{"keyword":"YOUR_KEYWORD","activity":"Activity Name","dedup":False}])
            csv_tpl = tpl_df.to_csv(index=False).encode()
            st.download_button("⬇️  Download CSV Template", csv_tpl, "mapping_template.csv", "text/csv", use_container_width=True)
    
    with tab_upload:
        mapping_file = st.file_uploader("Upload mapping CSV (columns: keyword, activity, dedup)", type=['csv'], key='mapping_upload')
        if mapping_file:
            try:
                ...
                if 'keyword' not in df_map.columns or 'activity' not in df_map.columns:
                    ...
                    st.success(f"✓ Loaded {len(st.session_state.mapping)} mapping rules")
            except Exception as e:
                st.error(f"Error: {e}")
    
    with tab_manual:
        st.markdown('<div style="font-size:0.82rem;color:#4a6278;margin-bottom:0.8rem;">Add mapping rules manually. Each rule maps a keyword in the log to an activity name.</div>', unsafe_allow_html=True)
        
        c1, c2, c3, c4 = st.columns([3, 3, 1, 1])
        with c1: new_kw  = st.text_input("Keyword (substring in log line)", key="new_kw", placeholder="e.g. BEGIN")
        with c2: new_act = st.text_input("Activity Name", key="new_act", placeholder="e.g. Start Batch")
        with c3: new_dd  = st.checkbox("Dedup", key="new_dd", help="Only record this activity once per case")
        with c4:
            st.write("")
            st.write("")
            if st.button("+ Add", use_container_width=True):
                if new_kw and new_act:
                    st.session_state.mapping.append({"keyword": new_kw, "activity": new_act, "dedup": new_dd})
                    st.rerun()
        ...
        if st.button("▶  PARSE EVENT LOG", use_container_width=True):
            if not st.session_state.mapping:
                ...
                with st.spinner("Parsing log file..."):
                    try:
                        df_raw = parse_log_with_mapping(st.session_state.log_text, st.session_state.mapping)
                        if df_raw.empty:
                            st.error("No events could be extracted. Check your keyword mapping.")
                        else:
...
if st.session_state.step >= 2 and st.session_state.df_raw is not None:
    ...
    st.session_state.log_stats = {
        'total_cases': n_cases, 'total_events': n_events,
        'unique_activities': n_acts, 'avg_acts_per_case': avg_acts,
        'date_start': str(ts_min.date()), 'date_end': str(ts_max.date()),
    }
    ...
    # Preview tabs
    tab_prev1, tab_prev2, tab_prev3 = st.tabs(["  Event Log Table", "  Activity Distribution", " "])
    ...
    with tab_prev2:
        ...
        fig_act = make_plotly_bar(act_counts, 'Count', 'Activity', 'Activity Frequency Distribution', h=max(300, min(600, len(act_counts)*28)))
        st.plotly_chart(fig_act, use_container_width=True)
    
    with tab_prev3:
        ts_daily = df.groupby(df['time:timestamp'].dt.date).size().reset_index()
        ts_daily.columns = ['Date', 'Event Count']
        fig_ts = px.area(ts_daily, x='Date', y='Event Count',
            title='Daily Event Volume',
            color_discrete_sequence=['#00d4ff'])
        fig_ts.update_layout(
            paper_bgcolor='#151b24', plot_bgcolor='#151b24',
    ...
    # Compute duration for downstream analysis
    ...
    with col_run2:
        if st.button("▶  RUN FULL ANALYSIS", use_container_width=True):
            with st.spinner("Running bottleneck analysis..."):
                st.session_state.bottleneck_df  = compute_bottleneck_table(df_dur)
...
if st.session_state.step >= 3:
    ...
    with main_tab1:
        ...
        st.markdown('<div style="font-size:0.85rem;color:#8fa3bc;">Extracted process model from event log using Heuristic Miner algorithm.</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        with col_d2:
            ...
            case_dur = df.groupby('case:concept:name').agg(
                start=('time:timestamp','min'), end=('time:timestamp','max')
            ).reset_index()
            case_dur['duration_sec'] = (case_dur['end'] - case_dur['start']).dt.total_seconds()
            case_dur = case_dur[case_dur['duration_sec'] > 0]
            
            fig_dur = px.histogram(case_dur, x='duration_sec', nbins=40,
    ...
    yaxis=dict(
        ...
        with col_viz2:
            ...
            st.markdown(f"""<div class="pm-card">
    ...
    with main_tab2:
        ...
        st.markdown('<div style="font-size:0.85rem;color:#8fa3bc;">Compare actual process (event log) against reference model (BPMN). Upload your BPMN file to run this analysis.</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        
        bpmn_file = st.file_uploader("Upload BPMN Reference Model (.bpmn)", type=['bpmn','xml'], key='bpmn_up')
        ...
        if bpmn_file:
            ...
            with col_conf1:
                if st.button("▶  Run Conformance Checking", use_container_width=True):
                    with st.spinner("Running Token-Based Replay & Alignment Analysis..."):
                        try:
                            fitness, precision, deviations = run_conformance(df, bpmn_bytes)
        ...
        if st.session_state.fitness is not None:
            ...
            fit_pct  = fitness.get('perc_fit_traces', 0)
            
            # Gauges
            col_g1, col_g2, col_g3 = st.columns(3)
            with col_g1:
                st.plotly_chart(fitness_gauge(fit_val, "Average Trace Fitness"), use_container_width=True)
            with col_g2:
                st.plotly_chart(fitness_gauge(prec_val, "Precision"), use_container_width=True)
            with col_g3:
                st.plotly_chart(fitness_gauge(fit_pct/100 if fit_pct > 1 else fit_pct, "% Fit Traces"), use_container_width=True)
            
            # Metrics detail
            col_fm1, col_fm2 = st.columns(2)
            with col_fm1:
                st.markdown("#### Token-Based Replay Metrics")
                token_data = {
                    'Metric': ['Average Trace Fitness','% Traces Fully Fit','Produced Tokens','Consumed Tokens','Missing Tokens','Remaining Tokens'],
                    'Value': [
                        f"{fit_val:.4f}",
                        f"{fitness.get('perc_fit_traces',0):.1f}%",
                        str(fitness.get('produced_tokens','N/A')),
                        str(fitness.get('consumed_tokens','N/A')),
                        str(fitness.get('missing_tokens','N/A')),
                        str(fitness.get('remaining_tokens','N/A')),
                    ],
                    'Status': [
                        '🟢 Good' if fit_val>=0.8 else '🟡 Medium' if fit_val>=0.5 else '🔴 Low',
                        '🟢 Good' if fitness.get('perc_fit_traces',0)>=80 else '🟡 Medium' if fitness.get('perc_fit_traces',0)>=50 else '🔴 Low',
                        '—','—',
                        '🟢 Low' if fitness.get('missing_tokens',999)<100 else '🔴 High',
                        '🟢 Low' if fitness.get('remaining_tokens',999)<100 else '🔴 High',
                    ]
                }
            ...
            with col_fm2:
                if deviations:
                    ...
                    dev_df = pd.DataFrame(deviations[:15], columns=['Deviation', 'Frequency'])
                    dev_df['Deviation'] = dev_df['Deviation'].str[:60]
                    st.dataframe(dev_df, use_container_width=True, height=300)
                    
                    total_log_moves   = sum(v for k,v in deviations if '>>' in k.split('|')[0])
                    total_model_moves = sum(v for k,v in deviations if '>>' in k.split('|')[1] if '|' in k)
                    st.markdown(f"""<div class="pm-card">
            ...
            st.info("Upload a BPMN file and click 'Run Conformance Checking' to see results here.")
    ...
    with main_tab3:
        ...
        st.markdown('<div style="font-size:0.85rem;color:#8fa3bc;">Multi-dimensional performance analysis based on inter-event time (duration_ms). Each metric reveals a different aspect of system performance.</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        
        if bn_df is not None and not bn_df.empty:
            sub1, sub2, sub3, sub4 = st.tabs([
                " Total Time Ranking", " Frequency Ranking", " Mean Duration Ranking", " Transition Delays"
            ])
            
            with sub1:
                st.markdown("**Activities ranked by cumulative total time — highest contributors to system load**")
                col_b1, col_b2 = st.columns([1, 1])
                with col_b1:
                    display_cols = ['concept:name','frekuensi','total_waktu','rata_rata','median','p95','maks','pct_total','variasi']
                    st.dataframe(bn_df[display_cols].head(15).rename(columns={
                        'concept:name':'Activity','frekuensi':'Freq',
                        'total_waktu':'Total (ms)','rata_rata':'Mean (ms)',
                        'median':'Median (ms)','p95':'P95 (ms)',
                        'maks':'Max (ms)','pct_total':'% Total','variasi':'Volatility'
                    }), use_container_width=True, height=400)
                with col_b2:
                    fig_bn = make_plotly_bar(bn_df.head(10), 'total_waktu', 'concept:name',
                        'Top Bottleneck — Total Time (ms)', h=400)
            ...
            with sub2:
                ...
                with col_f1:
                    st.dataframe(top_freq[['concept:name','frekuensi','rata_rata','pct_total']].head(10).rename(columns={
                        'concept:name':'Activity','frekuensi':'Frequency','rata_rata':'Mean (ms)','pct_total':'% Total Time'
                    }), use_container_width=True, height=350)
                with col_f2:
                    fig_fr = make_plotly_bar(top_freq.head(10), 'frekuensi', 'concept:name',
                        'Top Activities by Execution Frequency', h=350)
            ...
            with sub3:
                ...
                col_d1, col_d2 = st.columns([1,1])
                with col_d1:
                    st.dataframe(top_dur[['concept:name','rata_rata','median','p95','variasi']].head(10).rename(columns={
                        'concept:name':'Activity','rata_rata':'Mean (ms)',
                        'median':'Median (ms)','p95':'P95 (ms)','variasi':'Volatility'
                    }), use_container_width=True, height=350)
                with col_d2:
                    fig_dr = make_plotly_bar(top_dur.head(10), 'rata_rata', 'concept:name',
                        'Top Activities by Mean Duration (ms)', h=350)
            ...
            with sub4:
                if tr_df is not None and not tr_df.empty:
                    ...
                    with col_t1:
                        st.dataframe(tr_df[['transition','frekuensi','rata_rata','median','p95']].head(15).rename(columns={
                            'transition':'Transition (A → B)','frekuensi':'Freq',
                            'rata_rata':'Mean Delay (ms)','median':'Median (ms)','p95':'P95 (ms)'
                        }), use_container_width=True, height=400)
                    with col_t2:
                        fig_tr = make_plotly_bar(tr_df.head(10), 'rata_rata', 'transition',
                            'Top Transition Delays — Mean (ms)', h=400)
            ...
            if not bn_df.empty:
                ...
                st.markdown(f"""<div class="pm-card" style="border-color:#2a3a50;margin-top:1rem;">
                    ...
                    <div style="font-size:0.9rem;color:#e2eaf4;line-height:1.7;">
            ...
            st.warning("No bottleneck data available. Ensure event log has valid timestamps.")
    ...
    with main_tab4:
        ...
        st.markdown('<div style="font-size:0.85rem;color:#8fa3bc;">Auto-generated recommendations based on bottleneck analysis and conformance results. Both technical and business dimensions.</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        for _, row in bpi_df.iterrows():
            ...
            color = '#ff4757' if 'Tinggi' in priority else '#ffb800' if 'Sedang' in priority else '#00ff9d'
            st.markdown(f"""<div class="pm-card" style="border-left:3px solid {color};margin-bottom:0.8rem;">
    ...
    with main_tab5:
        ...
        st.markdown('<div style="font-size:0.85rem;color:#8fa3bc;">Download analysis results in various formats.</div>', unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        
        col_e1, col_e2, col_e3 = st.columns(3)
        
        with col_e1:
            st.markdown("""<div class="pm-card" style="text-align:center;padding:2rem;">
            ...
            if bn_df is not None:
                ...
                st.download_button("⬇  Download Bottleneck CSV", csv_bn,
                    f"bottleneck_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv",
        ...
        with col_e2:
            ...
            if st.session_state.bpi_df is not None:
                ...
                st.download_button("⬇  Download BPI CSV", csv_bpi,
                    f"bpi_recommendations_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv",
                    use_container_width=True)
        ...
        with col_e3:
            ...
            summary_csv = export_summary_csv(
                ...
                st.session_state.bpi_df if st.session_state.bpi_df is not None else pd.DataFrame()
            )
            st.download_button("⬇  Download Full Summary", summary_csv,
                f"processmine_report_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv",
                use_container_width=True)
        ...
        if df is not None:
            ...
            st.download_button("⬇  Download Parsed Event Log (CSV)", csv_log,
                f"event_log_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv")
        ...
        if st.session_state.mapping:
            ...
            st.download_button("⬇  Download Mapping Config (CSV)", csv_map,
                "mapping_config.csv", "text/csv")
